...
async def delete_complain(complaints_id: int):
    await ComplaintManager.delete(complaints_id)


@router.put("/complaints/{complaints_id}/approve", dependencies=[Depends(oauth2_scheme), Depends(is_approver)],
            status_code=204)
async def approve_complaint(complaint_id: int):
    await ComplaintManager.approve(complaint_id)


@router.put("/complaints/{complaints_id}/reject", dependencies=[Depends(oauth2_scheme), Depends(is_approver)],
            status_code=204)
